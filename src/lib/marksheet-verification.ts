...
export function getFeeClearanceForCertificate({
  student,
...
export function studentRequestedMarksheetVerification(student: Student): boolean {
  return Boolean(student.marksheet_verification_requested_at);
}

/** Faculty may verify only after the student has requested verification from their portal. */
export function canFacultyVerifyMarksheet(student: Student, next: boolean): boolean {
  if (!next) return true;
  return studentRequestedMarksheetVerification(student);
}

/** Admin may approve only after a student request. */
export function canAdminVerifyMarksheet(student: Student, next: boolean): boolean {
  if (!next) return true;
  return studentRequestedMarksheetVerification(student);
}
...
export function getMarksheetEligibility({
  ...
  const adminOk = Boolean(student.admin_verified) && studentRequestedMarksheetVerification(student);
...
export function buildAdminVerificationUpdate({
  ...
  const eligibility = getMarksheetEligibility({
    student: { 
      ...student, 
      admin_verified: true,
      marksheet_verification_requested_at: student.marksheet_verification_requested_at || new Date().toISOString() 
    },
    hasMarksheet,
    hasLibraryPenalty,
  });
