...
/** Admin may approve only after a student request and faculty verification. */
export function canAdminVerifyMarksheet(student: Student, next: boolean): boolean {
  if (!next) return true;
  return studentRequestedMarksheetVerification(student) && Boolean(student.faculty_verified);
}
