...
describe("marksheet data helpers", () => {
  ...
  it("keeps the Aarav Sharma demo marksheet for 24btre148 with distinct courses and totals", () => {
    expect(AARAV_MARKSHEET_SEED.student_roll_no).toBe("24btre148");
    expect(AARAV_MARKSHEET_SEED.student_name).toBe("Aarav Sharma");
    expect(AARAV_MARKSHEET_SEED.semester_label).toBe("5");
    expect(AARAV_MARKSHEET_SEED.courses).toHaveLength(11);
    expect(AARAV_MARKSHEET_SEED.courses[0].course_code).toBe("24BTRE3511");
    expect(calculateMarksheetTotals(AARAV_MARKSHEET_SEED.courses)).toEqual({
      totalCredits: 25,
      totalCreditsEarned: 25,
      totalCreditPoints: 205,
      sgpa: 8.2,
      finalGrade: "A+",
    });
