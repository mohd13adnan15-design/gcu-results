...
const ASSET_PATHS = {
  ...
  logo: "/templates/assets/ChatGPT Image May 10, 2026, 11_53_51 PM.png",
...
function drawHeader(
  ...
  if (images.logo) {
    ...
    doc.addImage(images.logo.dataUrl, images.logo.type, logoX, y - 12, logoWidth, logoHeight);
  }
...
function drawMarksTable(
  ...
  for (const group of groupCoursesBySection(marksheet.courses)) {
    ...
    for (const course of group.courses) {
      const titleLines = fitLines(doc, course.course_title, widths[2] - 7, 2);
      const rowHeight = titleLines.length > 1 ? 24 : 18;
...
function drawTotals(doc: jsPDF, marksheet: StudentMarksheet, x: number, y: number, width: number) {
  ...
  setText(doc, RED);
  doc.text(`TOTAL CREDIT POINTS = ${formatNumber(marksheet.total_credit_points)}`, x + 7, y + 16);
  setStroke(doc);
  doc.setFontSize(12);
  doc.text(
  ...
  doc.text("GRADE :", x + width - gradeWidth + 16, y + 36);
  doc.setFontSize(14);
  doc.text(marksheet.final_grade, x + width - gradeWidth / 2 + 10, y + 36, { align: "center" });
  return y + height;
}
...
function drawFirstPageFooter(
  ...
  doc.setFontSize(11.5);
  setText(doc, DARK);
  doc.text(`Date : ${formatDate(new Date().toISOString())}`, pageWidth / 2, yEnd + 18, {
    align: "center",
  });

  if (images.seal) doc.addImage(images.seal.dataUrl, images.seal.type, 42, 705, 96, 96);
  ...
  return await new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        const isNearWhite = max > 228 && min > 210;
