...
export function StudentMarksReviewPanel({ studentId, portal }: Props) {
  ...
  async function reportIssue() {
    ...
    try {
      const fallback =
        portal === "admin_1"
          ? `Please recheck and edit marksheet data for ${student.full_name} (${student.student_id}).`
          : `Please recheck fees, library, hostel, and marksheet data for ${student.full_name} (${student.student_id}).`;

      const title =
        portal === "admin_1"
          ? "Admin 2 reported a marksheet mismatch"
      ...
      toast.success(portal === "admin_1" ? "Issue reported to Admin 1" : "Issue reported");
      setNote("");
      await load();
    } catch (error) {
    ...
    try {
      ...
      if (next) {
        await supabase.from("portal_notifications").insert({
          ...
          student_id: student.id,
          title: "Admin 2 marksheet verification complete",
          message: `${student.student_id} was verified by Admin 2.`,
        });
      }
  ...
  async function toggleAdminVerified(next: boolean) {
    ...
    if (next && !student.faculty_verified) {
      toast.error("Admin 2 verification must be completed before issuing grade card.");
      return;
    }
    setVerifyBusy(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <Link
        to={portal === "admin_1" ? "/admin-2" : "/admin-1"}
      ...
      <div className="card-elevated rounded-2xl p-6">
        ...
        <div className="mt-6">
          <p className="mb-3 text-sm font-semibold text-primary">
            Saved marksheet (student_marksheets)
          </p>
          <MarksheetSavedPreview marksheet={marksheet} readOnlyNotice />
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3 rounded-xl border border-border bg-cream p-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-semibold text-primary">
              {portal === "admin_1"
                ? "Admin 2 · Marksheet verification"
                : "Admin 1 · Final approval"}
            </p>
            <p className="text-xs text-muted-foreground">
              {portal === "admin_1"
                ? "Verify and issue grade card after checking saved marksheet."
                : "Approve after reviewing Admin 2 verification."}
            </p>
          </div>
          {portal === "admin_1" ? (
            <button
        ...
        <div className="mt-6 space-y-3 rounded-xl border border-border bg-cream p-4">
          <label className="block text-sm font-medium text-primary">
            {portal === "admin_1" ? "Report mismatch to Admin 1" : "Report issue"}
            <textarea
              value={note}
...
function VerificationSummary({
  ...
  return (
    <div className="rounded-xl border border-border bg-secondary/30 px-4 py-3 text-sm">
      ...
      <p>
        Admin 2: <strong>{student.faculty_verified ? "Verified" : "Pending"}</strong>
      </p>
      {portal === "admin_2" && (
        <p>
          Admin 1: <strong>{student.admin_verified ? "Issued" : "Pending"}</strong>
        </p>
      )}
      {pending.length > 0 ? (
