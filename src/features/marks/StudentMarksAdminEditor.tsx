...
export function StudentMarksAdminEditor({
  ...
  const load = useCallback(async () => {
    ...
    const nextHeader: HeaderFormState = {
      ...
      card_student_name: headerRow?.student_name?.trim() || currentStudent?.full_name || "",
      ...
      school_name: sheetExtra?.school_name ?? "SCHOOL OF ENGINEERING AND TECHNOLOGY",
  ...
  return (
    <div className={shellClass}>
        ...
        <div className="mb-2 rounded-2xl border-2 border-primary/30 bg-gradient-to-b from-white to-cream/80 p-5 shadow-md">
          ...
          <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            ...
            <div className="rounded-xl border border-border bg-secondary/25 px-4 py-3 text-sm">
              ...
              <p>
                Credits: <strong>{totalCreditsEarned.toFixed(1)}</strong> /{" "}
          ...
          <div>
            ...
            {!compact ? (
              <p className="text-sm text-muted-foreground">
                Same fields as the Excel template (24btre152-style). Add every subject row here or
                use bulk upload on the Super Admin home page.
              </p>
            ) : null}
          </div>
      ...
      {prettyCard ? (
        ...
        <div
          className={
            compact ? "mt-4 space-y-3" : "mt-4 rounded-xl border border-border bg-cream p-4"
          }
        >
