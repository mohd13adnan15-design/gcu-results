...
type MarksheetSavedPreviewProps = {
  ...
  /** Admin 1 / Admin 2 review: show copy that marks cannot be edited on this screen. */
  readOnlyNotice?: boolean;
};

/** Admin 1/2 marksheet preview (via fetchStudentMarksheet → `student_marksheets`). */
    ...
    <div>
      {readOnlyNotice ? (
        <p className="mb-4 rounded-lg border border-border bg-secondary/30 px-3 py-2 text-sm text-foreground">
          <strong>View only.</strong> This table is the saved marksheet record in the database. To
          change marks or header lines, Admin 2 must use{" "}
          <strong>Admin 2 → Students → Marks</strong> (edit there).
        </p>
      ) : null}
