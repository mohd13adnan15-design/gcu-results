...
export function SuperAdminStudentHub({ studentId }: Props) {
  ...
  const clearVerification = async () => {
    if (!student) return;
    if (!confirm("Reset Admin 1, Admin 2, and final verification for this student?")) return;
    const { error } = await supabase
      .from("students")
      .update({
  ...
  const approveGradeCard = async () => {
    ...
    toast.success("Admin 1 approved this grade card.");
    void load();
  };
  ...
  if (!student) {
    return (
      <div className="card-elevated rounded-2xl p-8 text-center">
        ...
        >
          <ArrowLeft className="h-4 w-4" /> Back to Admin 1
        </Link>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 border-b border-border pb-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Admin 1 home
          </Link>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {student.email} · {student.department} · Semester {student.semester} · Year{" "}
            {student.year}
          </p>
        ...
        <div className="flex shrink-0 flex-col gap-2 rounded-xl border border-border bg-cream/60 p-4 text-sm">
          ...
          <div className="flex flex-wrap gap-2">
            <StatusPill ok={Boolean(student.faculty_verified)} label="Admin 2" />
            <StatusPill ok={Boolean(student.admin_verified)} label="Issued" />
            <StatusPill ok={Boolean(student.fully_verified)} label="Final" />
          </div>
          <button
            type="button"
            onClick={() => void approveGradeCard()}
            className="mt-1 rounded-md border border-primary/30 bg-primary/10 px-2 py-1 text-left text-xs font-medium text-primary hover:bg-primary/15"
          >
            Approve
          </button>
          <button
        ...
        <div className="space-y-6">
          ...
          >
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Update the student record used across all portals. Check which areas this student
                belongs to (fees, hostel, library lists).
              </p>
            </div>
      ...
      {tab === "fees" && (
        ...
        >
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Match the numbers used on the dedicated fees and hostel portals. Check the box when
      ...
      {tab === "marks" && (
        <div className="rounded-2xl border-2 border-primary/25 bg-cream p-6 shadow-sm">
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            ...
            <strong className="text-foreground">Save header &amp; sync</strong> to push grades and
            SGPA into the saved marksheet (
            <code className="rounded bg-muted px-1 py-0.5 text-xs">student_marksheets</code>) for
            Admin and students.
          </p>
          <div className="mt-5">
