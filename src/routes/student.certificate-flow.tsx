...
function CertificateFlow() {
      ...
      try {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            /* Clear stale flags so the flow matches this request (legacy rows often had faculty pre-checked). */
        ...
        await supabase.from("portal_notifications").insert({
          ...
          title: "Student requested marksheet verification",
  ...
  const flowNodes: {
    ...
    },
    {
      ...
      short: "Verification review",
      ok: facultyOk,
      pendingHint:
        feesOk && hasRequest && !facultyOk
    ...
    },
    {
      ...
      short: "Approval",
      ok: adminOk,
      pendingHint: facultyOk && !adminOk ? "After Verification 1 review" : !facultyOk ? "After Verification 1" : undefined,
    },
    {
      key: "pdf",
      title: "Certificate",
      short: "Download PDF",
      ok: pdfOk,
      pendingHint: !pdfOk ? "After Verification 2 approves" : undefined,
    },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="card-elevated rounded-2xl border border-border bg-cream/80 p-6 md:p-8">
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
                Use <strong>Generate certificate</strong> once — we check fees, queue Verification 1 and
                Verification 2 automatically, and unlock PDF download when every step is green. Status
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 rounded-xl border border-amber-200/80 bg-amber-50/90 px-4 py-3 text-sm text-amber-950">
          ...
          <span>
            <strong>Review timing:</strong> Verification 1 and Verification 2 together share one{" "}
            <strong>48-hour</strong> target from when you start (not 48 hours each). No extra
            buttons — one start, then wait for green checks.
          </span>
        </div>
      </section>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {hasRequest
                  ? "Verification 1 and Verification 2 are working through the steps above. This page updates automatically."
                  : "One tap submits your file to Verification 1, then Verification 2 — no separate request step."}
              </p>
            </div>
            <button
              type="button"
