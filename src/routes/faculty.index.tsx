...
import { Eye, Loader2, MessageSquareWarning, ShieldCheck } from "lucide-react";
...
export function FacultyPage() {
  ...
  async function toggleFacultyVerified(
    ...
    if (next && !student.marksheet_verification_requested_at) {
      toast.error("This student has not requested marksheet verification from their portal yet.");
      return;
    }
    ...
    try {
      ...
      toast.success(next ? "Admin 2 verification complete" : "Admin 2 verification removed");
      await load();
    } catch (error) {
  ...
  async function reportIssue(student: Student) {
    setBusyStudentId(student.id);
    try {
      const title = "Marksheet issue reported";
      if (
      ...
      const { error: notificationError } = await supabase.from("portal_notifications").insert({
        ...
        title: "Admin 2 reported a marksheet mismatch",
      ...
      toast.success("Issue reported to Admin 1");
      await load();
    } catch (error) {
  ...
  async function issueGradeCard(
    student: Student,
    marksheet: StudentMarksheet | null,
    legacyCourseCount: number,
  ) {
    const hasMarksheetData = Boolean(marksheet) || legacyCourseCount > 0;
    if (!student.faculty_verified) {
      toast.error("Complete Admin 2 verification before issuing grade card.");
      return;
    }
    setBusyStudentId(student.id);
    try {
  ...
  return (
    <div className="card-elevated rounded-2xl p-6">
      <h2 className="text-xl font-bold text-primary">Admin 2 verification queue</h2>
      <p className="mt-1 text-sm text-muted-foreground">
        Only students who requested verification from their portal appear here. Review marks from{" "}
        <strong>student_marksheets</strong> or subject rows in <strong>student_marks</strong> (bulk
        upload), verify, then click <strong>Issue grade card</strong>. If you find any mismatch, use{" "}
        <strong>Report</strong> to send it to Admin 1 for edits.
      </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-muted-foreground">
                ...
                <th className="px-2 py-2">Admin 2</th>
                  ...
                  <tr key={student.id} className="border-b border-border/60">
                    ...
                    <td className="px-2 py-3">
                      <input
                        ...
                        aria-label={`Admin 2 verification for ${student.student_id}`}
                      />
                    </td>
                    <td className="px-2 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          type="button"
                          onClick={() => void issueGradeCard(student, marksheet, legacyCourseCount)}
                          disabled={busyStudentId === student.id || !student.faculty_verified}
                        ...
                        <Link
                          to={`/admin-2/students/${student.id}`}
          ...
          {filteredRows.length === 0 && (
            <p className="mt-6 text-center text-sm text-muted-foreground">
              {students.length > 0 && rows.length === 0
                ? "No students have requested marksheet verification yet. They submit the request from their portal after clearing fees."
