      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {row.student_id ? (
              <Link
                to={`/admin-1/students/${row.student_id}`}
                ...
                return (
                  <tr key={student.id} className="border-b border-border/60">
                    ...
                    <td className="px-2 py-2 text-right">
                      <Link
                        to={`/admin-1/students/${student.id}`}
...
function MarksUploader() {
  ...
  function downloadTemplate() {
    ...
    const readme = XLSX.utils.aoa_to_sheet([
      ...
      ["• You can also add or edit marks on the site: Admin 1 → student → Subject marks."],
    ]);
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (validRows.length === 0) {
        toast.error(
          "No valid rows: need Student ID, Email, Name, Course Code, Title, and credits > 0.",
        );
        return;
      }
