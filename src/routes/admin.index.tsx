...
export function AdminPage() {
  ...
  async function markAdminVerified(
    ...
    if (next && !adminReady) {
      toast.error("Student is not ready for Admin 2 verification.");
      return;
    }

    if (next && !canAdminVerifyMarksheet(student, true)) {
      toast.error(
        "Admin 2 verification is only available after the student requests verification and Admin 1 approves.",
      );
      return;
    }
  ...
  return (
    <div className="card-elevated rounded-2xl p-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Only students who requested verification from their portal appear here. Admin 1 and Admin 2
        share a combined <strong>48-hour</strong> review target per student. Review payments,
        library, hostel, and the saved marksheet before Admin 2 verification — after Admin 1 has
        approved.
      </p>
      ...
      <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-end">
        ...
        <label className="flex min-w-[180px] flex-col gap-1 text-xs font-medium text-muted-foreground">
          ...
          >
            ...
            <option value="pending_faculty">Pending Admin 1</option>
            <option value="pending_admin">Pending Admin 2</option>
            <option value="admin_done">Admin 2 verified</option>
          </select>
        </label>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-muted-foreground">
                ...
                <th className="px-2 py-2">Admin 1</th>
          ...
          {filteredRows.length === 0 && (
            <p className="mt-6 text-center text-sm text-muted-foreground">
              {students.length > 0 && rows.length === 0
                ? "No students have requested marksheet verification yet."
